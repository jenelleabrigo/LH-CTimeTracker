...
class Application
{
    ...
    function displayTemplate($template, $assignedValue = array())
    {
        ...
        if (!file_exists($cacheFile) || filemtime($cacheFile) < filemtime($templateFile)) {
            ...
            $pattern = array(
                ...
                '/\{\{\s*foot\s*\}\}/i' => '<?php if (isset($foot)) { $assignedClass[\'app\'] = FEGG_getInstance(); foreach($foot as $key => $value) { $assignedClass[\'app\']->setCurrentTemplateDirectory($value[\'dir\']); $assignedClass[\'app\']->displayTemplate($value[\'file\'], $assignedValue); } } ?>',
                '/\{\{\s*include\s+foot\s+(\'*[^\s]+\'*)\s*\}\}/i' => '<?php if (isset($foot)) { array_unshift($foot, array( \'file\'=> $1, \'dir\'=>\''.$currentDir.'\' )); } else { $foot[] = array( \'file\'=> $1, \'dir\'=>\''.$currentDir.'\' ); } ?>',
    ...
    function in($name = '', $method = '')
    {
        ...
        $requestData = !$this->_empty($requestData) ? $requestData : "";
